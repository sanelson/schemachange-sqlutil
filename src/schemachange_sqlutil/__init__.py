...
from schemachange_sqlutil.deploy import deployment
import dataclasses
...
class SchemachangePlugin(Plugin):
    def __init__(self, *args, **kwargs):
        ...
        self.plugin_description = "Custom utility plugin for schemachange"
...
#
#    #    plugin_name = "sqlutil_plugin"
#    plugin_subcommand = "sqlutil"
#    plugin_description = "SQL utilities plugin for schemachange"
...
class RerunPluginConfig(PluginJobConfig):
    ...
    plugin_description = "Rerun repeatable scripts plugin for schemachange"
    #    plugin_parent_arguments = []
    plugin_subcommand_arguments = [
        {
            "name_or_flags": [
...
class DeployPluginConfig(PluginBaseConfig, DeployConfig):
    ...
    plugin_name = "DeployPlugin"
    plugin_subcommand = "deploy"
    plugin_description = "Custom Deployment plugin for schemachange"
    plugin_parent_arguments = []
    plugin_subcommand_arguments = [
        {
            "name_or_flags": [
    ...
    def plugin_run(self, session):
        logger.info("Running DeployPlugin")

        # Create a new DependentDeployment instance
        dependent_deployment = deployment.DependentDeployment(
            config=self, session=session
        )

        dependent_deployment.run()
        return
...
#        logger.info("Running Post-Deploy Plugin Tasks")
#
#        if self.run_deps:
#            logger.info("Rerunning dependent scripts")
#
#        return
